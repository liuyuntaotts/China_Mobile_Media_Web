				...
				<table class="table table-bordered" style="margin-bottom:0px;">
					<tbody>
						<tr>
							...
							<td width="320px">
							{{if status=='unchecked'}}
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=checked&id={{:id}}" class="linkbtn">审核通过</a>
								<a href="../views/videoPoint.jsp?id={{:id}}" class="linkbtn">关联商品</a>
								<a href="../views/videoEdit.jsp?id={{:id}}" data-fancybox-type="iframe" class="btn-edit linkbtn">编辑</a>
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/delete?id={{:id}}" class="linkbtn">删除</a>								
							{{else status=='checked'}}
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=unchecked&id={{:id}}" class="linkbtn">取消审核</a>
								<a href="../views/videoPoint.jsp?id={{:id}}" class="linkbtn">关联商品</a>
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/delete?id={{:id}}" class="linkbtn">删除</a>
								
							{{else status=='transcoding'}}
								<button onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=uploaded&id={{:id}}" class="linkbtn">重新处理</a>
							{{else status=='failed'}}
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=uploaded&id={{:id}}" class="linkbtn">重新处理</a>
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=destroy&id={{:id}}" class="linkbtn">彻底删除</a>
							{{else status=='deleted'}}
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=unchecked&id={{:id}}" class="linkbtn">恢复</a>
								<a onclick="changeStatus(this)" data-id="{{:id}}" data-url="../rest/videoinfo/changeStatus?status=destroy&id={{:id}}" class="linkbtn">彻底删除</a>
							{{/if}}
							</td>
						</tr>
                ...
                <div class="page-content">
					...
					<!--content-->
						<div class="content">
							...
							<hr>
							<div class="cui-menu">
								<a class="btn-create btn" href="../views/videoAdd.jsp">
									上传视频
								</a>
							</div>
							<!--搜索框-->
